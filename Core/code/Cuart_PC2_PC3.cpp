...
short Cuart_PC2_PC3::U_to_DAC8563(int16_t U,const int8_t a)
{
	...
	long DAC;
	DAC = (U * 30638L) / 10000;  // разрядность 32768 / макс напряжение 10695 мВольт ,0.695 В уходит на смещение
	DAC += 32768;
	#if (configUSE_PLC == 1)
	if(DAC > 3 && DAC < 65533)
		DAC += (flash_prg._Allp_param7.U_off_a[a] % 10) / 3; // прибавляем младший разряд смещений 0.000 0
	#endif
	return (short)DAC;
}
